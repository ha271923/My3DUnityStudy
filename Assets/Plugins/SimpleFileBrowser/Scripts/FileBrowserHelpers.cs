...
namespace SimpleFileBrowser
{
	...
	{
		private static string LOG_TAG = "My3DUnityStudy::";
		...
		private static string TemporaryFilePath
		{
			get
			{
				...
				Debug.LogWarning(LOG_TAG+"TemporaryFilePath=" + TemporaryFilePath);
		...
		{
			bool ret = ShouldUseSAF && ( string.IsNullOrEmpty( path ) || path[0] != '/' );
			Debug.LogWarning(LOG_TAG+"ShouldUseSAFForPath   ret=" + ret);
			return ret;
		}
#endif
		...
		{
			bool ret = false;
			Debug.LogWarning(LOG_TAG + "DirectoryExists + path=" + path);
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) ) {
				ret = AJC.CallStatic<bool>( "SAFEntryExists", Context, path, false );
				Debug.LogWarning(LOG_TAG+"DirectoryExists - ret="+ret);
				return ret;
			}
#endif
		...
		public static bool DirectoryExists( string path )
		{
...
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) ) {
				ret = AJC.CallStatic<bool>( "SAFEntryExists", Context, path, true );
				Debug.LogWarning(LOG_TAG + "DirectoryExists - ret="+ret);
				return ret;
			}
			else if( ShouldUseSAF ) // Directory.Exists returns true even for inaccessible directories on Android 10+, we need to check if the directory is accessible
			{
				try
				{
					...
					Debug.LogWarning(LOG_TAG + "DirectoryExists - true");
					return true;
				}
				catch
				{
					Debug.LogWarning(LOG_TAG + "DirectoryExists - false");
					return false;
				}
			}
#endif
		...
		{
			bool ret = false;
			Debug.LogWarning(LOG_TAG + "IsDirectory + path=" + path);
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) ) {
				ret = AJC.CallStatic<bool>( "SAFEntryDirectory", Context, path );
				Debug.LogWarning(LOG_TAG + "IsDirectory - ret=" + ret);
				return ret;
			}
#endif
			if (Directory.Exists(path))
			{
				Debug.LogWarning(LOG_TAG + "IsDirectory - true");
				return true;
			}
			if (File.Exists(path))
			{
				Debug.LogWarning(LOG_TAG + "IsDirectory - false");
				return false;
			}

			string extension = Path.GetExtension( path );
			ret = extension == null || extension.Length <= 1; // extension includes '.'
			Debug.LogWarning(LOG_TAG + "IsDirectory - extension=" + extension+"   ret="+ret);
			return ret;
		}
		...
		{
			string ret = null;
			Debug.LogWarning("GetDirectoryName + path="+path);
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) ) {
				ret = AJC.CallStatic<string>( "GetParentDirectory", Context, path );
				Debug.LogWarning("GetDirectoryName - ret="+ret);
				return ret;
			}
			else
				Debug.LogWarning("GetParentDirectory API error");
			Debug.LogWarning("GetDirectoryName - ret=" + ret);
#endif
		...
		{
			Debug.LogWarning("GetEntriesInDirectory +  path="+path+ "      extractOnlyLastSuffixFromExtensions="+ extractOnlyLastSuffixFromExtensions);
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) )
			{
				...
				printFileDirectory(result);
				Debug.LogWarning("GetEntriesInDirectory -");
				return result;
			}
#endif
		...
		static private void printFileDirectory(FileSystemEntry[] files) {

			foreach (FileSystemEntry file in files)
			{
				Debug.LogWarning("IsDirectory=" + file.IsDirectory + "     Name=" + file.Name);
			}
		}
		...
		{
			string ret = null;
			Debug.LogWarning("CreateFileInDirectory +  path="+ directoryPath+ "   filename="+ filename);
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( directoryPath ) ){
				ret = AJC.CallStatic<string>( "CreateSAFEntry", Context, directoryPath, false, filename );
				Debug.LogWarning("CreateFileInDirectory -   ret = " + ret);
				return ret;
			}
#endif
			...
			Debug.LogWarning("CreateFileInDirectory -   path = "+ path);
			return path;
		}
		...
		{
			string ret = null;
			Debug.LogWarning("CreateFolderInDirectory +   path = " + directoryPath +"    folderName = "+folderName);
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( directoryPath ) ) {
				ret = AJC.CallStatic<string>( "CreateSAFEntry", Context, directoryPath, true, folderName );
				Debug.LogWarning("CreateFolderInDirectory -   ret = " + ret);
				return ret;
			}
#endif
			...
			Debug.LogWarning("CreateFolderInDirectory -   path = " + path);
			return path;
		}
		...
		{
			string ret = null;
			Debug.LogWarning("GetFilename +   path = " + path);
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) ) {
				ret = AJC.CallStatic<string>( "SAFEntryName", Context, path );
				Debug.LogWarning("GetFilename -   ret = " + ret);
				return ret;
			} else {
				Debug.LogWarning("GetFilename -   ret = " + ret);
				return ret;
